...
from core.history import bucket_of, state_key  # noqa: E402
...
class FakeStore:
    """Counts reads, because "one bucket read per bucket" is a design promise."""
...
def store_with(provider: str, company: str, *, tracked_since: str, jobs: dict) -> FakeStore:
    key = state_key(bucket_of(provider, company))
    return FakeStore(
        {
            key: {
                "companies": {
...
async def test_a_tracked_live_job_answers_from_one_bucket_read():
    store = store_with(
        "ashby",
        "ramp",
    ...
    row = await Enricher(store=store, today=TODAY).enrich(f"https://jobs.ashbyhq.com/ramp/{UUID}")
...
async def test_the_boards_own_posted_date_beats_our_first_sighting():
...
async def test_a_one_day_window_cannot_read_as_never_closed():
    ...
    row = await Enricher(store=store, today=TODAY).enrich(f"https://jobs.lever.co/acme/{UUID}")
    assert row["historyWindowDays"] == 1
    assert row["firstSeenCensored"] is True
    assert row["ageBasis"] == "observed_floor", "a floor must not be presented as an age"


async def test_the_four_ways_of_not_knowing_stay_distinct():
    store = store_with(
        "lever",
        "acme",
        tracked_since="2026-08-26",
    ...
    enricher = Enricher(store=store, today=TODAY)
...
async def test_a_missing_id_on_a_watched_board_is_not_a_removal():
    ...
    row = await Enricher(store=store, today=TODAY).enrich(
...
async def test_buckets_are_read_once_per_bucket():
    store = store_with(
        "lever",
        "acme",
        tracked_since="2026-08-26",
        jobs={UUID: {"first_seen": "2026-08-26"}},
    )
    enricher = Enricher(store=store, today=TODAY)
    for _ in range(5):
        await enricher.enrich(f"https://jobs.lever.co/acme/{UUID}")
    assert store.reads == 1, "a dataset of thousands of rows must not be thousands of reads"
