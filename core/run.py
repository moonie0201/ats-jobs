"""The shared Actor pipeline every listing calls (SPEC v2 §4, §5.12, §8, §9.1, §13.4, §14).

Every Actor under ``actors/`` is a ~30-line ``src/main.py`` that calls :func:`main` here.
The per-ATS listings (A2–A4, §3.2) pass ``provider=`` to pin themselves to one ATS: the
provider selector disappears from their input schema, a bare token in ``companies``
becomes that provider's own board slug, and an entry naming a *different* ATS becomes a
free error row instead of a silent cross-provider fetch. Nothing else differs — same
adapters, same normalization, same `job` / `delta-run` events, same price.
...
def read_config(raw: dict[str, Any], provider: str | None = None) -> dict[str, Any]:
    """Apply defaults without letting a meaningful falsy value (`maxJobs: 0`) be lost.

    ``provider`` is the §3.2 pin. It overrides `providers` outright — the per-ATS input
    schemas have no selector, so anything arriving under that key came from an API caller
    replaying a multi-ATS input, and honouring it would deliver another ATS's jobs from a
    listing that names one. The delta store is namespaced with it too: the named KV store
    is account-wide, so a shared default would have the Greenhouse listing marking ids
    that then went missing from the multi-ATS listing's own delta.
    """
    cfg = {
    ...
    if provider:
        ...
        if raw.get("stateKey") is None:
            cfg["stateKey"] = f"{provider}-jobs-state-default"
    else:
...
class RunCtx:
    ...
    filters: Filters
    billing: Billing
    #: §3.2 single-ATS pin, or None on the multi-ATS Actor.
...
async def resolve_all(entries: list[str], client: Any, ctx: RunCtx) -> list[Ref]:
    ...
    """
    directory = None
    if any(needs_directory(entry, ctx.provider) for entry in entries):
        try:
    ...
    for entry in entries:
        ...
        result = resolve(
            entry, providers=ctx.cfg["providers"], directory=directory, pin=ctx.provider
        )
...
async def main(provider: str | None = None) -> None:
    """Run one Actor. ``provider`` pins it to a single ATS (§3.2); None = all six."""
    async with Actor:
        raw = await Actor.get_input() or {}
        cfg = read_config(raw, provider)
